...
def semeval_scorer(predict_label, true_label, class_num=19):
    import math
    ...
    p = np.zeros(shape=[class_num], dtype=np.float32)
    r = np.zeros(shape=[class_num], dtype=np.float32)

    for i in range(0, class_num):  # ignore the 'Other'
        try:
            p[i] = float(confusion_matrix[i][i]) / float(col_sum[i] + xDIRx[i])
            r[i] = float(confusion_matrix[i][i]) / float(row_sum[i] + xDIRx[i])
...
class Eval(object):
    ...
    def evaluate(self, model, criterion, data_loader):
        ...
        with torch.no_grad():
            model.eval()
            for _, (data, label, bert_logits) in enumerate(data_loader):
        ...
        f1, p, r = semeval_scorer(predict_label, true_label)
        return f1, eval_loss, predict_label, p, r
