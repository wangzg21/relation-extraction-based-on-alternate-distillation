...
class SemEvalDateset(Dataset):
    def __init__(self, filename, rel2id, word2id, config):
        ...
        self.data_dir = config.data_dir
        self.dataset, self.label, self.bert_logits = self.__load_data()
    ...
    def __symbolize_sentence(self, e1_pos, e2_pos, sentence):
        ...
        if e1_pos[0] < e2_pos[0]:
            for i in range(e1_pos[0], min(e2_pos[1]+1, len(sentence))):
                mask[i] = 2
            for i in range(min(e2_pos[1]+1, len(sentence)), len(sentence)):
                mask[i] = 3
        else:
            for i in range(e2_pos[0], min(e1_pos[1]+1, len(sentence))):
                mask[i] = 2
            for i in range(min(e1_pos[1]+1, len(sentence)), len(sentence)):
                mask[i] = 3
    ...
    def __load_data(self):
        ...
        bert_logits_file = os.path.join(self.data_dir, 'berttestdata.txt')
        bert_logits = np.loadtxt(bert_logits_file)
        # print(bert_logits[:3])
        return data, labels, bert_logits

    def __getitem__(self, index):
        data = self.dataset[index]
        label = self.label[index]
        bert_logits = self.bert_logits[index]
        return data, label, bert_logits
...
class SemEvalDataLoader(object):
    ...
    def __collate_fn(self, batch):
        data, label, bert_logits = zip(*batch)  # unzip the batch data
        data = list(data)
        label = list(label)
        bert_logits = list(bert_logits)
        data = torch.from_numpy(np.concatenate(data, axis=0))
        label = torch.from_numpy(np.asarray(label, dtype=np.int64))
        bert_logits = torch.from_numpy(np.asarray(bert_logits, dtype=np.float32))
        return data, label, bert_logits
